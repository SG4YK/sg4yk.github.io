---
layout: post
title: 'Get motion sensors' data of DualShock 4 using the new Input System'
subtitle: ' '
date: 2020-04-03
categories: 'Tech'
tags: Unity
...
There're 2 ways to create a custom layout, using struct or JSON. In this tutorial I'd to use JSON for convenience.
...
I suggest you to modify the `name` and the `displayName` so that it can be differed from the default one.  

![](/assets/img/post/2020/04/custom_layout.png)

Go through evry item under `control`, and refering to the report structure provieded by [eleccelerator wiki](http://eleccelerator.com/wiki/index.php?title=DualShock_4#Report_Structure), the customized controls should look like this.   

![](/assets/img/post/2020/04/example.png)  

Well, some of you may be confused that why the `layout` field should be `Button` rather than something like `Axis`. In general a real button only has 2 status, released and pressed, representing 0 and 1 as value.
For an axis, it should ba a 1D vector ranged from -1 to 1 or 0 to 1.  
...
The data returned from a stick and a gyroscope should be similar, the only defference is that a gyroscope has one more axis. So, for every axis of the gyroscope or the accelerometer, just copy a button layout under L or R stick, delete unused fields, modify the offset, and that's it.  
...
Thank you for reading this artical and don't forget to comment below if you find any mistake.
